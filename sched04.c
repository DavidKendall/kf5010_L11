...
int main(void) {
    ...
    assert(rc == 0);
    rc = pthread_mutexattr_getprotocol(&mutex_attr, &result);
    assert((rc == 0) && (result == PTHREAD_PRIO_PROTECT));
    rc = pthread_mutexattr_setprioceiling(&mutex_attr, BASE_PRIORITY + N_THREADS);
    assert(rc == 0);
    rc = pthread_mutexattr_getprioceiling(&mutex_attr, &result);
    assert((rc == 0) && (result == BASE_PRIORITY + N_THREADS));
